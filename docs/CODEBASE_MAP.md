---
last_mapped: 2026-02-06T15:36:37Z
total_files: 8
total_tokens: 9094
---
...
> Auto-generated by Cartographer. Last mapped: 2026-02-06 (manually updated; re-run Cartographer for full accuracy)

## System Overview

CLI tool for delegating tasks to Codex (execution) and Gemini (chat/UX) agents via tmux sessions. Designed for Claude Code orchestration with bidirectional communication.
...
│   ├── gemini-runner.ts  # Interactive Gemini runner (tmux pane) (new)
│   ├── jobs.ts          # Job lifecycle management (1,905 tokens)
│   ├── session-parser.ts # Parse Codex sessions for jobs --json
...
| `createSession(options)` | Create detached session with provider (codex|gemini) |
